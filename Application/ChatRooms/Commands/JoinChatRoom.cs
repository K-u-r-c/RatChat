        ...
        public async Task<Result<ChatRoomIdentifierDto>> Handle(Command request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                if (parts.Length == 2)
                {
                    ...
                    if (invite == null) return Result<ChatRoomIdentifierDto>.Failure("Invalid join token", 401);
                ...
                else if (parts.Length >= 3)
                {
                    ...
                    legacyExpiry = DateTime.Parse(expiryString, null, DateTimeStyles.RoundtripKind);
                }
                else
                {
            ...
            if (legacyExpiry.HasValue && !string.IsNullOrWhiteSpace(chatRoomIdFromToken) &&
                chatRoom.Id != chatRoomIdFromToken)
                return Result<ChatRoomIdentifierDto>.Failure("Invalid join token", 401);
            ...
            var ban = chatRoom.Bans.FirstOrDefault(b => b.UserId == user.Id && b.ChatRoomId == chatRoom.Id);

            if (ban != null) return Result<ChatRoomIdentifierDto>.Failure("User is banned from this chat room", 401);
            ...
            if (invite != null)
            {
                ...
                if (!string.IsNullOrEmpty(invite.AllowedUserId)) invite.MaxUses = 1;
            ...
            if (!saved) return Result<ChatRoomIdentifierDto>.Failure("Problem updating the DB", 400);
