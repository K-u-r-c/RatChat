...
import {buildDesktopDeepLink} from "../../lib/oauth/getOAuthRedirectUrl";
...
export default function AuthCallback() {
  ...
  const desktopBridgeRequested = params.get("desktop") === "1";
  const fetched = useRef(false);

  const desktopDeepLink = useMemo(() => {
    if (!provider || !code) return null;
    return buildDesktopDeepLink(provider, code);
  }, [provider, code]);

  useEffect(() => {
    if (desktopBridgeRequested && !isDesktopRuntime) {
      return;
    }
  ...
  }, [desktopBridgeRequested, desktopDeepLink]);
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            If the app does not open automatically, you can close this window and switch back to RatChat.
          </Typography>
        </Paper>
      </Box>
    );
