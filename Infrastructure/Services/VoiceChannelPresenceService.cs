...
{
    private readonly ConcurrentDictionary<string, ConcurrentDictionary<string, VoiceParticipantConnection>> _channels = new();
    ...
    {
        if (!_connections.TryRemove(connectionId, out var participant))
        {
            return null;
        }
        ...
        if (channel.IsEmpty)
        {
            _channels.TryRemove(participant.ChannelId, out _);
        }
    ...
    public IReadOnlyCollection<VoiceParticipantConnection> GetParticipants(string channelId)
    {
        ...
        {
            return Array.Empty<VoiceParticipantConnection>();
        }
